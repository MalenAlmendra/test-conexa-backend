...
export class MoviesServicesMock {
  async createMovie(createMovieDto: CreateMovieDTO): Promise<MoviesEntity> {
    return Promise.resolve({
      idMovie: (Math.random() * (1000 - 1) + 1).toString(),
      ...createMovieDto,
    });
  }

  async updateMovie(updateMovieDto: UpdateMovieDto): Promise<string> {
    return Promise.resolve({
      ...updateMovieDto,
    }) as Promise<any>;
  }
  ...
  async getAllMovies(): Promise<MoviesResponseDTO> {
    const moviesList: MoviesResponseDTO = {
      count: 1,
      next: null,
      previous: null,
      results: [
  ...
  async getMovieDetails(idMovie): Promise<Result> {
    let movie: Result = {
    ...
    return Promise.resolve({ idMovie: idMovie, ...movie });
  }
}
